...
router.get("/posting", async (req, res) => {
  ...
  future.then(result => {
    res.send(JSON.stringify({ "message": result, "status": 200, "error": false }));
  }).catch(error => {
...
router.post("/posting", async (req, res) => {
  ...
  future.then(result => {
    res.send(JSON.stringify({ "message": JSON.stringify(result), "status": 200, "error": false }));
  }).catch(error => {
    res.status(500).send("Data base: " + error);
  });
...
router.put("/posting/:idPosting", async (req, res) => {
  ...
  future.then(result => {
    ...
    res.status(500).send({ "message": "Data base error" + error, "status": 500, "error": true });
  });
});
